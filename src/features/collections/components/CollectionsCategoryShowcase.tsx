...
import type { ShowcaseCategoryItem } from "@/services/catalog-public-service";

type CollectionsCategoryShowcaseProps = {
  apiCategories?: ShowcaseCategoryItem[];
};

export function CollectionsCategoryShowcase({ apiCategories }: CollectionsCategoryShowcaseProps) {
        ...
        {apiCategories.map((category, index) => {
          ...
          return (
            <div key={category.id} className="grid grid-cols-1 lg:grid-cols-2 min-h-[80vh]">
              ...
              <div
                className={`flex flex-col justify-center px-8 py-20 lg:px-24 ${isEven ? "lg:order-2" : "lg:order-1"}`}
              >
                <motion.div
                ...
                >
                  {category.subtitle && (
                    <span className="text-primary text-xs uppercase tracking-widest font-medium block mb-4">
                      {category.subtitle}
                    </span>
                  )}
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                    ...
                    <span className="text-sm font-serif italic text-muted-foreground">
                      {category.productCount} pièces uniques
                    </span>
                  </div>
                </motion.div>
              </div>
